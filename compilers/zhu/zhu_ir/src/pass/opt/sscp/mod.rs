...
use crate::entities::constant::ConstantData;
use crate::entities::function::Function;
use crate::entities::immediate::{compute_binary_immi, compute_unary_immi, Immediate};
use crate::entities::instruction::{opcode::OpCode, Instruction, InstructionData};
use crate::entities::value::{Value, ValueData};
use crate::pass::analysis::use_chain::UseTable;
    ...
    fn compute_lattice_over_inst(&self, inst: Instruction, function: &Function) -> Lattice {
        ...
        match inst_data {
            ...
            InstructionData::Unary { opcode, value } => match self.lattices.get(value).unwrap() {
                ...
                Lattice::Value(immi) => Lattice::Value(compute_unary_immi(*opcode, immi.clone())),
            },
            ...
            InstructionData::BinaryI { opcode, value, imm } => match self.lattices.get(value).unwrap() {
                Lattice::Bottom => Lattice::Bottom,
                Lattice::Top => Lattice::Top,
                Lattice::Value(arg_immi) => Lattice::Value(compute_binary_immi(*opcode, arg_immi.clone(), imm.clone())),
            },
            InstructionData::Binary { opcode, args } => {
                ...
                match (arg1_lattice, arg2_lattice) {
                    ...
                    (Lattice::Value(immi1), Lattice::Value(immi2)) => {
                        Lattice::Value(compute_binary_immi(*opcode, immi1.clone(), immi2.clone()))
                    }
                }
            }
